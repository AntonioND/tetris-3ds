...
while(time != 1320)
	...
	if(time < 720)	
		{
		glTranslatef((float)cos((float)time) * ((720 - (float)time) / 720),(float)sin((float)time) * ((720 - (float)time) / 720),0);
		glRotateZ(time+=2); 
		...
		{
		time++;
		if(Pad.Newpress.Anykey || Stylus.Newpress) time = 1320;
		}
	...
	glTexCoord2f(0,1);  glVertex3f(-6, 2,-6); 
	glTexCoord2f(1,1);  glVertex3f( 6, 2,-6);
	...
	glTexCoord2f(1,0);  glVertex3f( 6, 2,-6); 
	glTexCoord2f(0,0);  glVertex3f(-6, 2,-6);
	glEnd(); 
...
u8 a; for (a = 0; a < 32; a++) 
	{
	PA_SetBrightness(0, -a); PA_SetBrightness(1, -a);
