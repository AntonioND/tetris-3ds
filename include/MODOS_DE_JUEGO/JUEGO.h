...
else if(Stylus.Vx > -2) stylus_usedgravity = 0;
...
else if(Stylus.Vx < 2) stylus_reservausada = 0;


if(Stylus.Released) { Stylus.X = 128; Stylus.Y = 92; }
}
...
else if(Stylus.Vy < 2) stylus_usedgravity = 0;
...
else if(Stylus.Vy > -2) stylus_reservausada = 0;
...
if((timepressed % 5 == 0) && (timepressed > 10))
	{
	if(lastkeypressed == PAD_DOWN) Pad.Newpress.Down = 1;
	else if(lastkeypressed == PAD_UP) Pad.Newpress.Up = 1;
	}
}
...
if((timepressed % 5 == 0) && (timepressed > 10))
	{
	if(lastkeypressed == PAD_DOWN) Pad.Newpress.Right = 1;
	else if(lastkeypressed == PAD_UP) Pad.Newpress.Left = 1;
	}
}
...
void Ajustar_Colisiones_Actuales()
{
for(auxiliar = 0;auxiliar < 16; auxiliar++) Colisiones_Actual[auxiliar] = 0;
switch(Tipo_Pieza[0])
	{
	case 0: //Pieza 0
...
else return (Limitar_int(0,1,Escenario_Tetris[x][y].dibujar - 1));
}
...
if(auxiliar2 == 1)
	...
	if(Comprobar_Colisiones_Izquierda() == 1)
		...
		auxiliar2 = 4;
		Pieza.X --;
		if(Tipo_Pieza[0] == 0) Pieza.X --; //si es la alargada prueba una posicion mas.
...
for(auxiliar = 0;auxiliar < ALTO_PIEZAS + 2; auxiliar++) Fila_Eliminada[auxiliar] = 0;

//Preparar piezas
for(auxiliar = 0;auxiliar < 5; auxiliar++) Tipo_Pieza[auxiliar] = PA_RandMinMax(0,6);
...
for(auxiliar = 0;auxiliar < 4; auxiliar++) Tipo_Pieza[auxiliar] = Tipo_Pieza[auxiliar + 1];
Tipo_Pieza[4] = PA_RandMinMax(0,6);

Ajustar_Colisiones_Actuales();

for(auxiliar = 1;auxiliar <= 4; auxiliar++) PA_SetSpriteAnim(1, auxiliar, Tipo_Pieza[auxiliar]);
}
