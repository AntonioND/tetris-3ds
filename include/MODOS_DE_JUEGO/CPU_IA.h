...
void Set_Position_CPU() //Ayuda para IA_CPU()
{
...
for(Pieza_CPU.Rotacion = 0;Pieza_CPU.Rotacion < 4; Pieza_CPU.Rotacion++)
	...
	if(!Comprobar_Colisiones_Derecha_CPU())
		while(!Comprobar_Colisiones_Derecha_CPU()) Pieza_CPU.X ++; //Poner a la derecha
	...
	while(auxiliar4 < 2) //Hasta llegar a la izquierda
		...
		if(!Comprobar_Colisiones_Inferior_CPU())
			while(!Comprobar_Colisiones_Inferior_CPU()) Pieza_CPU.Y--;
		...
		if(Pieza_CPU.X > 0) Pieza_CPU.X--;
		else auxiliar4 = 2; //Si se sale
		
		if(Comprobar_Colisiones_Izquierda_CPU()) auxiliar4++;
		}
...
void Mover_IA_CPU()
{
if(activarreserva > 0) activarreserva--;
if(activarreserva == 1) Reservar_Pieza_CPU();
...
if((Pieza_CPU.Rotacion != Posicion_Ideal_CPU.Rotacion) && (framecount % 3 == 0))
	...
	switch(Comprobar_Colisiones_Giro_CPU())
		...
		case 4: //A la izquierda
			...
			Pieza_CPU.Rotacion = (Pieza_CPU.Rotacion + 1) % 4;
			Pieza_CPU.X --;
			if(Tipo_Pieza_CPU[0] == 0) Pieza_CPU.X --;
			break;
		}
	}
...
if((Pieza_CPU.X == Posicion_Ideal_CPU.X) && (Pieza_CPU.Rotacion == Posicion_Ideal_CPU.Rotacion)) Caida_rapida_CPU = 1;
else Caida_rapida_CPU = 0;
}
...
void Ajustar_Colisiones_Actuales_CPU()
{
for(auxiliar = 0;auxiliar < 16; auxiliar++) Colisiones_Actual_CPU[auxiliar] = 0;
switch(Tipo_Pieza_CPU[0])
	{
	case 0: //Pieza 0
...
else if (y > 16) return 0;
else return (Limitar_int(0,1,Escenario_Tetris_CPU[x][y].dibujar - 1));
}
...
if(auxiliar2 == 1)
	...
	if(Comprobar_Colisiones_Izquierda_CPU() == 1)
		...
		auxiliar2 = 4;
		Pieza_CPU.X --;
		if(Tipo_Pieza_CPU[0] == 0) Pieza_CPU.X --; //si es la alargada prueba una posicion mas.
...
switch(Tipo_Pieza_CPU[0]) //Color
	...
	case 0:
		...
		break;
	case 1:
		red_ = 1; green_ = 1; blue_ = 0;
		break;
	case 2:
		red_ = 1; green_ = 0.5; blue_ = 0;
		break;
	case 3:
		red_ = 1; green_ = 0; blue_ = 0;
		break;
	case 4:
		red_ = 0; green_ = 1; blue_ = 0;
		break;
	case 5:
		red_ = 0; green_ = 0; blue_ = 1;
		break;
	case 6:
		red_ = 0.75; green_ = 0; blue_ = 1;
		break;
	}
...
for(auxiliar = 0;auxiliar < ALTO_PIEZAS + 2; auxiliar++) Fila_Eliminada_CPU[auxiliar] = 0;

//Preparar piezas
for(auxiliar = 0;auxiliar < 5; auxiliar++) Tipo_Pieza_CPU[auxiliar] = PA_RandMinMax(0,6);
...
for(auxiliar = 0;auxiliar < 4; auxiliar++) Tipo_Pieza_CPU[auxiliar] = Tipo_Pieza_CPU[auxiliar + 1];
Tipo_Pieza_CPU[4] = PA_RandMinMax(0,6);

Ajustar_Colisiones_Actuales_CPU();

for(auxiliar = 1;auxiliar <= 4; auxiliar++) PA_SetSpriteAnim(1, auxiliar + 50, Tipo_Pieza_CPU[auxiliar]);
}
